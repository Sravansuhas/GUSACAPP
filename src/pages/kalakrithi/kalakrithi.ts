...
@Component({
  selector: 'page-kalakrithi',
  templateUrl: 'kalakrithi.html',
  providers:[Api]
})
export class KalakrithiPage {
	datas:any;
	type:boolean=false;
	pagination:number=1;
	searchKeyword:any="";
  ...
  constructor(public navCtrl: NavController,private api:Api,private toast:ToastController) {
	api.index_kalakrithi(1).subscribe(data => {
	...
	doRefresh(ev){
		this.api.index_kalakrithi(1).subscribe(data=>{
			ev.complete();
			this.searchKeyword="";
			this.pagination=1;
			this.type=false;
